...
public class SPlacementFragment {
    final int rowWidth = 4;

    public SuenoCategory currentCategory = SuenoCategory.distribution;

    Seq<Block> returnArray = new Seq<>(), returnArray2 = new Seq<>();
    Seq<SuenoCategory> returnCatArray = new Seq<>();
    boolean[] categoryEmpty = new boolean[SuenoCategory.all.length];
    ObjectMap<SuenoCategory,Block> selectedBlocks = new ObjectMap<>();
    ObjectFloatMap<SuenoCategory> scrollPositions = new ObjectFloatMap<>();
    ...
    public SPlacementFragment(){
        Events.on(WorldLoadEvent.class, event -> {
            Core.app.post(() -> {
                currentCategory = SuenoCategory.distribution;
    ...
    boolean gridUpdate(InputHandler input){
        ...
        if(Core.input.keyTap(Binding.pick) && player.isBuilder() && !Core.scene.hasDialog()){ //mouse eyedropper select
            ...
            if(tryRecipe != null && tryRecipe.isVisible() && unlocked(tryRecipe)){
                ...
                currentCategory = SuenoCategory.valueOf(input.block.category.toString());
                return true;
            }
        }
        ...
        for(int i = 0; i < blockSelect.length; i++){
            if(Core.input.keyTap(blockSelect[i])){
                if(i > 9){ //select block directionally
                    ...
                    if(!getUnlockedByCategory(SuenoCategory.all[i]).isEmpty()){
                        currentCategory = SuenoCategory.all[i];
                        if(input.block != null){
            ...
            full.table(frame -> {
                ...
                //blockCatTable: all blocks | all categories
                {
                    ...
                    blockCatTable.table(categories -> {
                        ...
                        for(SuenoCategory cat : SuenoCategory.all){
                        ...
                        int f = 0;
                        for(SuenoCategory cat : getCategories()){
    ...
    Seq<SuenoCategory> getCategories(){
        return returnCatArray.clear().addAll(SuenoCategory.all).sort((c1, c2) -> Boolean.compare(categoryEmpty[c1.ordinal()], categoryEmpty[c2.ordinal()]));
    }

    Seq<Block> getByCategory(SuenoCategory cat){
        return returnArray.selectFrom(content.blocks(), block -> SuenoCategory.valueOf(block.category.toString()) == cat && block.isVisible() && block.environmentBuildable());
    }

    Seq<Block> getUnlockedByCategory(SuenoCategory cat){
        return returnArray2.selectFrom(content.blocks(), block -> SuenoCategory.valueOf(block.category.toString()) == cat && block.isVisible() && unlocked(block)).sort((b1, b2) -> Boolean.compare(!b1.isPlaceable(), !b2.isPlaceable()));
    }

    Block getSelectedBlock(SuenoCategory cat){
