...
public class SModsDialog extends ModsDialog {
    ...
    public SModsDialog(){
        ...
        browser.resized(this::rebuildBrowser);
    ...
    private void showMod(LoadedMod mod){
        ...
        dialog.cont.pane(desc -> {
            ...
            desc.row();
            if (mod.main == SVars.core){
                desc.add(
                        Strings.format("[#@]Sueno", Colorated.applyToColor2(Interp.linear, Color.white, Color.darkGray, Color.white, (Time.time/1f) % 1f).toString().substring(0, 6))
                ).growX().wrap().padTop(2);
            } else {
